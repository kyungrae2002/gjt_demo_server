...
- 신청번호: 문서에 인쇄된 접수번호/문서번호를 글자 그대로 정확히 읽는다. 보이지 않으면 반드시 빈 문자열("").
  추측하지 말고, 매 문서마다 실제로 보이는 값을 다시 읽는다.
- 다른 필드도 값이 안 보이면 빈 문자열("")로 둔다.
- 품목 표의 각 행을 items 배열의 원소로 만든다. 합계/소계 같은 요약 행은 제외한다.
- 수량은 정수. 신청일자는 가능하면 YYYY-MM-DD 형식으로 정규화한다.
"""
