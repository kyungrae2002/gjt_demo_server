...
PARAMS = {
    ...
    # JSON에 투입인원수가 없거나 비어 있을 때만 사용하는 fallback 값
    "default_dispatch_people": 2,

    # 요청 건수가 이 값 이하이면 정확 TSP, 초과하면 NN + 2-opt
    "exact_tsp_threshold": 8,

    # edge 비용
    # walk는 length를 사용하고, stair/elevator는 아래 고정 비용을 사용한다.
...
def split_location_to_building_room(location: str) -> tuple[str, str | None]:
    """
    설치장소를 첫 번째 공백 기준으로 분리한다.
    예: "외국어대학관 124" -> ("외국어대학관", "124")
        "공학실험동 211-3" -> ("공학실험동", "211-3")
    """
...
def build_graph(nodes: pd.DataFrame, edges: pd.DataFrame) -> nx.Graph:
    ...
    for idx, row in edges.iterrows():
        ...
        # walk edge만 length/distance를 숫자로 사용한다.
        # stair/elevator의 length/distance는 숫자가 아니어도 오류를 내지 않는다.
        length = 1.0
...
def decide_stair_policy(G: nx.Graph, grouped: pd.DataFrame, dispatch_people: float) -> dict:
    """
    엘리베이터가 하나라도 있는 건물에만 계단 회피 정책 적용.
    - 필요 인원수 2 이상 물품이 있거나
    - 물품 필요 인원수 최댓값 > 투입인원수이면
      계단을 금지하지 않고, 계단 edge 사용 개수를 최우선으로 최소화한다.
    - 연속 elevator 사용도 가능한 줄인다.
    """
...
def constrained_dijkstra(
    ...
    """
    우선순위 metric = (계단 edge 사용 수, 연속 elevator 발생 수, 기본 이동 비용)
    - avoid_stairs=True이면 계단 edge 사용 수를 최우선으로 최소화한다.
    - avoid_stairs=False이면 계단 수는 0으로 취급하고 기본 비용 중심으로 탐색한다.
    - avoid_consecutive_elevators=True이면 elevator edge를 연속으로 타는 경우를 최소화한다.
    """
    source = str(source)
    target = str(target)
...
def result_to_public_dict(result: dict) -> dict:
    ...
    return {
        ...
        # "총비용": result["total_cost"],
        # "사용계단엣지수": result["total_stair_edges"],
        # "연속엘리베이터발생수": result["total_consecutive_elevators"],
        "총방문노드순서": result["full_node_path"],
        "수거노드순서": result["pickup_node_order"],
        "수거호수순서": result["pickup_room_order"],
        "수거노드": pickup_nodes,
        # "계단정책": result["stair_policy"],
    }
...
def main(params: dict = PARAMS):
    ...
    print(" - building_routes_result.json")
    print(" - building_route_summary.csv")
    print(" - building_full_node_path.csv")
    print(" - building_pickup_nodes.csv")
    print(" - building_route_details.csv")
    print(" - unmatched_requests.csv (매핑 실패가 있을 때만 생성)")
    print("=" * 60)
...
# ============================================================
def run_optimizer(df: pd.DataFrame, _df_avail: pd.DataFrame, dispatch_people: float | None = None) -> list[dict]:
    """
    api.py에서 호출하는 진입점.
    ...
    for building, building_items in schedule.groupby("_building"):
        ...
        results.append(result_to_public_dict(result))
